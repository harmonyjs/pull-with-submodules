...
export async function stashExists(
  ...
  try {
    ...
    config.logger?.debug(
      `Unexpected error checking stash existence: ${errorMessage}`,
    );
...
export function validateStashReference(stashRef: string): void {
  if (stashRef.length === 0 || stashRef.trim().length === 0) {
    throw new GitOperationError("Stash reference cannot be empty", {
  ...
  const isValidFormat = /^(stash@\{\d+\}|[a-f0-9]{7,40})$/i.test(
    stashRef.trim(),
  );

  if (!isValidFormat) {
    throw new GitOperationError(`Invalid stash reference format: ${stashRef}`, {
 ...
 */
export function handleStashRestoreError(
  error: unknown,
  stashRef: string,
): never {
  const errorMessage = error instanceof Error ? error.message : String(error);
  ...
  if (errorMessage.includes("not a valid reference")) {
    throw new GitOperationError(`Stash ${stashRef} no longer exists`, {
      cause: error as Error,
      suggestions: [
  ...
  // Generic error
  throw new GitOperationError(`Failed to restore stash ${stashRef}`, {
    cause: error as Error,
    suggestions: [
